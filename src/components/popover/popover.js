...
MdPopoverService.prototype.create = function(scope, element, attrs) {
  ...
  if (popoverRef._config.scope.mdEnabled) {
    ...
    this._config.scope.$on('$destroy', angular.bind(popoverRef, popoverRef._destroy));
    this._config.element.one('$destroy', angular.bind(popoverRef, popoverRef._destroy));
    this._config.parentElement.one('$destroy', angular.bind(popoverRef, popoverRef._destroy));
  }
...
MdPopoverRef.prototype._configureCloseTriggers = function() {
  ...
  var handlerFn = function() {
    ...
    if (self._autohide || self._$document[0].activeElement !== self._config.parentElement[0]) {
...
MdPopoverRef.prototype._setVisible = function(isVisible) {
  ...
  if (!this._setVisible.queued) {
    if (isVisible) {
      ...
      this._openTimeout = this._$timeout(openHandlerFn, self._config.scope.mdOpenDelay);
    } else {
      this._closeTimeout = this._$timeout(closeHanderFn, self._config.scope.mdCloseDelay);
    }
  }
};
...
MdPopoverRef.prototype._open = function() {
  ...
  var panelClass, panelConfig, contentEl, children;
